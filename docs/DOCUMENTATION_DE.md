...
- **`notify.send_message`** (der generische, entitätsbasierte Dienst): trage die **Entity-ID** des gewünschten Empfängers ein, z. B. `notify.smtp_jane_doe`. Das wird als echtes Ziel-Selector-Feld übergeben, so wie es die moderne Notify-Plattform erwartet.
...
Beim ersten Start nach dem Update auf v1.3.0 zeigt HA möglicherweise eine Warnung für `sensor.event_history`: die Einheit `events` weicht von der zuvor gespeicherten leeren Einheit ab.
...
### Warnung „State attributes … exceed maximum size of 16384 bytes" für `sensor.event_history`

Bei vielen Geräten und/oder langer Event-Historie können die Attribute dieses Sensors die von Home Assistant für den Recorder erlaubte Maximalgröße von 16 KB überschreiten. HA protokolliert dann eine Warnung und speichert die Attribute für diesen State nicht in der Datenbank.

Das ist unkritisch: Der Sensor selbst funktioniert weiterhin normal und liefert die vollständige Event-Liste live an Dashboards (z. B. flex-table-card) — nur die historische Aufzeichnung der Attribute in der Recorder-Datenbank entfällt, was für dieses rollierende 100-Event-Fenster ohnehin wenig Mehrwert bietet.

**Lösung:** Attribute dieses Sensors vom Recorder ausschließen, um die Warnung zu vermeiden — die Funktionalität bleibt vollständig erhalten:
...
- **Mesh-Protokolle (Zigbee/ZHA, Z-Wave) – verzögerte Statuserkennung:** Beide sind Mesh-Netzwerke ohne dauerhafte Verbindung. Ob ein Knoten noch "lebt", wird meist nur reaktiv geprüft — also nur dann, wenn tatsächlich mit dem Gerät kommuniziert wird (z. B. ein Befehl gesendet oder ein Ping ausgeführt wird) — statt über einen kontinuierlichen Heartbeat. Trennt man ein Gerät physisch vom Strom, kann der Status in Home Assistant deshalb noch lange als "verfügbar" (mit letztem bekannten Wert) stehen bleiben, bis das Gerät das nächste Mal aktiv angesprochen wird; auch ein fehlgeschlagener Befehl (z. B. "device did not respond") setzt die Entität nicht zwangsläufig auf `unavailable`. Connection Observer kann nur auf tatsächliche `state_changed`-Events reagieren — bleibt dieses Event aus, bleibt auch die Erkennung aus. **Workaround:** Entweder eine Automation einrichten, die kritische Geräte regelmäßig aktiv anpingt/rekonfiguriert (z. B. der Service `zwave_js.ping` bei Z-Wave JS), damit HA den Status zeitnah aktualisiert, oder wie bei passiven BLE-Geräten das [Watch-Label](#watch-label--eigene-offline-indikatoren) mit einem Template-Sensor nutzen, der den `last_updated`-Zeitstempel überwacht.
