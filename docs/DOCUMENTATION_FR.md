...
### Cible de notification *(à partir de la v1.3.11)*

Un champ optionnel à côté du service de notification. Depuis que Home Assistant 2026.7.x a modifié le fonctionnement de la plateforme `notify`, certaines intégrations (ex. SMTP, ou les bots Telegram avec plusieurs chats configurés) n'exposent plus une entité `notify.*` distincte par destinataire — une entité groupée envoie désormais à tous les destinataires configurés en même temps, sans possibilité d'en choisir un seul. Si votre service de notification fonctionne ainsi et que vous souhaitez atteindre uniquement un destinataire précis, indiquez ici sa valeur cible (ex. une adresse e-mail ou un identifiant de chat Telegram) — elle sera transmise en tant que champ `target` à chaque notification envoyée par cette intégration. Laissez vide si votre service de notification cible déjà exactement la bonne personne.
...
**Type :** Capteur | **Unité :** events | **Icône :** `mdi:history`

Indique le nombre total d'événements de déconnexion stockés.

**Attribut d'état `events` :** Liste des 100 derniers événements, du plus récent au plus ancien.

Chaque entrée contient : `device_name`, `area`, `protocol`, `disconnected_at`, `reconnected_at`, `still_offline`, `is_critical`.

Conçu pour les cartes de tableau de bord comme **flex-table-card** ou **mushroom-template-card**.
...
Au premier démarrage après la mise à jour vers v1.3.0, HA peut afficher un avertissement pour `sensor.event_history` : l'unité `events` diffère de l'unité vide précédemment enregistrée.
...
### Avertissement « State attributes … exceed maximum size of 16384 bytes » pour `sensor.event_history`

Avec de nombreux appareils et/ou un long historique d'événements, les attributs de ce capteur peuvent dépasser la taille maximale de 16 Ko autorisée par Home Assistant pour le recorder. HA enregistre alors un avertissement et ne stocke pas les attributs de cet état dans la base de données.

C'est sans gravité : le capteur continue de fonctionner normalement et fournit toujours la liste complète des événements en direct aux tableaux de bord (p. ex. flex-table-card) — seul l'enregistrement historique des attributs dans la base de données du recorder est omis, ce qui n'apporte de toute façon que peu de valeur pour cette fenêtre glissante de 100 événements.

**Solution :** Excluez les attributs de ce capteur du recorder pour éviter l'avertissement — la fonctionnalité reste entièrement préservée :
...
- **Protocoles maillés (Zigbee/ZHA, Z-Wave) – détection d'état retardée :** Les deux sont des réseaux maillés sans connexion persistante. La vivacité d'un nœud n'est généralement vérifiée que de manière réactive — c'est-à-dire uniquement lorsque l'appareil est réellement sollicité (envoi d'une commande, ou ping) — plutôt que via un signal de vie continu. Si vous débranchez physiquement un appareil, son état dans Home Assistant peut donc rester « disponible » (avec ses dernières valeurs connues) pendant longtemps, jusqu'à la prochaine sollicitation active ; une commande échouée (ex. « device did not respond ») ne fait pas non plus nécessairement passer l'entité à `unavailable`. Connection Observer ne peut réagir qu'à de véritables événements `state_changed` — si cet événement ne se produit jamais, la détection non plus. **Contournement :** Mettez en place une automatisation qui pingue/reconfigure régulièrement les appareils critiques (ex. le service `zwave_js.ping` pour Z-Wave JS) pour que l'état de HA reste à jour, ou, comme pour les appareils BLE passifs, utilisez la fonction [Watch label](#watch-label--indicateurs-hors-ligne-personnalisés) avec un capteur template qui surveille l'horodatage `last_updated`.
