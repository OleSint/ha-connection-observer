...
- **Mesh protocols (Zigbee/ZHA, Z-Wave) – delayed status detection:** Both are mesh networks with no persistent connection. Whether a node is still "alive" is generally only checked reactively — i.e. only when the device is actually communicated with (a command is sent, or it's pinged) — rather than through a continuous heartbeat. If you physically unplug a device, Home Assistant's state can therefore stay "available" (with its last known values) for a long time, until the device is next actively addressed; a failed command (e.g. "device did not respond") doesn't necessarily flip the entity to `unavailable` either. Connection Observer can only react to actual `state_changed` events — if that event never fires, detection won't happen either. **Workaround:** Either set up an automation that periodically pings/reconfigures critical devices (e.g. the `zwave_js.ping` service for Z-Wave JS) so HA's state stays current, or, similar to passive BLE devices, use the [watch label feature](#watch-label--custom-offline-indicators) with a template sensor that tracks the `last_updated` timestamp.
