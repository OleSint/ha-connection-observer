...
- **Mesh-protocollen (Zigbee/ZHA, Z-Wave) – vertraagde statusdetectie:** Beide zijn mesh-netwerken zonder persistente verbinding. Of een node nog "leeft" wordt meestal alleen reactief gecontroleerd — dus alleen wanneer daadwerkelijk met het apparaat wordt gecommuniceerd (een commando wordt verzonden, of het wordt gepingd) — in plaats van via een continue heartbeat. Als je een apparaat fysiek loskoppelt van de stroom, kan de status in Home Assistant daardoor lange tijd "beschikbaar" (met de laatst bekende waarden) blijven, tot het apparaat de volgende keer actief wordt aangesproken; ook een mislukt commando (bijv. "device did not respond") zet de entiteit niet per se op `unavailable`. Connection Observer kan alleen reageren op daadwerkelijke `state_changed`-events — blijft dat event uit, dan blijft ook de detectie uit. **Workaround:** Stel een automatisering in die kritieke apparaten periodiek pingt/herconfigureert (bijv. de service `zwave_js.ping` voor Z-Wave JS) zodat de status in HA actueel blijft, of gebruik, net als bij passieve BLE-apparaten, de [Watch label](#watch-label--aangepaste-offline-indicatoren)-functie met een template-sensor die de `last_updated`-tijdstempel bewaakt.
